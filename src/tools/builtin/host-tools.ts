...
export const hostSysinfoTool = defineTool({
  name: 'host_sysinfo',
  description: '获取当前运行 Hermes Agent Platform 的宿主主机实时系统状态（CPU 型号/核心数/使用率、物理内存/进程内存使用情况、操作系统版本、连续运行时间、局域网 IP 等）。',
  schema: z.object({}),
  run: async () => {
    ...
    const output = [
      ...
      `- **系统运行时间**: ${formatUptime(info.os.uptimeSeconds)} (Hermes Agent Platform 连续运行: ${formatUptime(info.os.processUptimeSeconds)})`,
      ``,
      `⚡ **CPU 状态**:`,
      `- 型号: \`${info.cpu.model}\``,
      ...
      `- Hermes Agent Platform 进程占用 (RSS): \`${formatBytes(info.memory.processRssBytes)}\``,
